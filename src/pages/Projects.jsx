...
export default function Projects() {
  ...
  let filteredProjects = projects.filter((project) =>
    project.technologies.some((tech) => isSelected.includes(tech))
  );
  ...
  return (
    <motion.section
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="flex text-[14px] lg:flex-col h-full overflow-hidden scroll-none"
    >
      ...
      <div className="border-r-2 border-custom w-[18.6rem] h-full lg:hidden">
        <h2 className="flex items-center px-2 border-b-2 border-custom text-[#FFFFFF] py-1">
          <i className="ri-arrow-down-s-fill text-xl"></i>projects
        </h2>
        {tech.map((a, i) => (
          <div key={a} className="pl-3 flex mt-2 gap-4 items-center">
            ...
            />
            <span
              className={` hover:text-[#FFFFFF] pt-1 ${
                isSelected.includes(a) && "text-[#FFFFFF]"
              }`}
            >
              {a}
            </span>
      ...
      <div className="hidden lg:block">
            ...
            {tech.map((a, i) => (
              <div key={a} className="pl-3 flex mt-2 gap-4 items-center">
                <input
                  onChange={filter}
                  className="hover:outline"
                  defaultChecked={isSelected.includes(a)}
                  type="checkbox"
                  name={a}
                  id=""
                />
        ...
        <div className="py-4 px-5">
          <p className="mb-7 pr-2 lg:text-[#FFFFFF] lg:text-[16px] inline">
            {"//"} Projects:
          </p>
          ...
          <div className="border-b-2 border-custom lg:hidden w-full">
            <h2 className="flex items-center justify-between w-fit  px-2  border-r-2 border-custom py-1">
              {filteredProjects.length > 0
                ...
                : "all.js"}
              <i className="ri-close-line text-xl pl-5"></i>
            </h2>
          </div>
